...
import { getWorklogs } from 'apiSdk/worklogs';
import { WorklogInterface } from 'interfaces/worklog';
import { getJobs } from 'apiSdk/jobs';
import { JobInterface } from 'interfaces/job';

import { HiringListPage as ShowCaseList } from 'pages/hirings';
...
function DashboardPage() {
  ...
  const worklogStat = useSWR<PaginatedInterface<WorklogInterface>>(
    () => `/worklogs`,
    () => getWorklogs({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Worklogs',
      result: worklogStat?.data?.totalCount,
      isLoading: worklogStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Jobs',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`hirings`} passHref legacyBehavior>
                  <Button
